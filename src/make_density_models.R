...
if (!identical(st_crs(p, parameters = T)$proj4string, crs(npp, proj = TRUE))) {
   warning("CRS are not the same - reprojecting")
   p <- st_transform(p, crs = st_crs(crs(npp, proj = TRUE)))
}
...
# calculate the environmental part of the linear models ------
# this is useful so we can just save/return a table rather than
# rasters on which to compute the full linear models for each pixel.
#amphibians
...
amph_env <- project(amph_env, template)
bird_env <- project(bird_env, template)
mamm_env <- project(mamm_env, template)
rept_env <- project(rept_env, template)
